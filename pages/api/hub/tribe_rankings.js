...
export default async function handler(req, res) {
  ...
  if(current_page < 69) {
    next_page = `https://bloody.gg/api/hub/tribe_rankings?page=${parseInt(current_page) + 1}&search=${search}`;
  }

  if(current_page > 0) {
    prev_page = `https://bloody.gg/api/hub/tribe_rankings?page=${parseInt(current_page) - 1}&search=${search}`;
  }
